...
// Shift yang benar-benar dipakai satu bag. Kalau bag punya kolom `shifts` custom
// (mis. Tangerang yang cuma buka Pagi & Siang, tanpa Malam), pakai itu -- supaya
// baseline generator tidak terus mencoba mengisi shift yang memang tidak
// dioperasikan di bag tsb (dan bikin tabel jadwal penuh "kosong" percuma).
function bagShifts(bag) {
  if (bag.shifts && String(bag.shifts).trim()) {
    return String(bag.shifts)
      .split(',')
      .map((s) => s.trim())
      .filter((s) => SHIFTS.includes(s));
  }
  return DEPT_SHIFTS[bag.department] || SHIFTS;
}
...
/**
 * ===================== PRIORITAS HARI LIBUR (Senin-Kamis dulu) =====================
 * Kesepakatan HR: hari libur diusahakan jatuh di Senin-Kamis dulu. Kalau kapasitas
 * tidak cukup (jatah libur tidak muat semua di Senin-Kamis), baru boleh geser ke
 * Jumat, lalu Sabtu, lalu Minggu (urutan terakhir). Ini berlaku untuk semua
 * departemen (termasuk Host Live yang idealnya sama sekali tidak libur weekend).
 * Angka lebih kecil = lebih diprioritaskan sebagai hari libur.
 */
function weekdayRestCost(dateStr) {
  const day = new Date(dateStr + 'T00:00:00').getDay(); // 0=Minggu ... 6=Sabtu
  if (day >= 1 && day <= 4) return 0; // Senin-Kamis
  if (day === 5) return 1; // Jumat
  if (day === 6) return 2; // Sabtu
  return 3; // Minggu
}

/**
 * ===================== BLACKOUT EVENT (23-25 & tanggal kembar) =====================
 * Setiap tanggal 23-25 tiap bulan (event bulanan) dan "tanggal kembar"
 * (mis. 8.8, 9.9 -- termasuk bulan lain seperti 10.10/11.11/12.12) dianggap
 * masa event. Selama masa event, hari libur diusahakan TIDAK jatuh di tanggal
 * tsb (biasanya window tanggal kembar = 2 hari, H-1 & H, mis. 8.8 -> 7-8 Agt).
 */
function isEventBlackout(dateStr) {
  const d = new Date(dateStr + 'T00:00:00');
  const day = d.getDate();
  const month = d.getMonth() + 1;
  if (day >= 23 && day <= 25) return true; // event bulanan 23-25
  if (day === month || day === month - 1) return true; // tanggal kembar (H-1 & H)
  return false;
}
...
function isAllowed(hostId, date, shift, bagId, placed, approvedLeaveDates, weeklyShiftCount, maxShiftsByHost) {
  ...
  // Aturan jumping: tidak boleh Siang -> besok Pagi, dan tidak boleh Malam ->
  // besok Pagi atau Siang (istirahat kurang), lintas bag.
  if (yesterdayData?.shifts.has('S') && shift === 'P') return false;
  if (yesterdayData?.shifts.has('M') && (shift === 'P' || shift === 'S')) return false;
...
function assignRestDaysForWindow(dates, deptHosts, approvedLeaveDates, preferredDates) {
  ...
  for (const h of order) {
    ...
    while (need > 0) {
      ...
      // Urutan prioritas hari libur (nilai lebih kecil = lebih diutamakan):
      //  1. Senin-Kamis dulu; kalau tidak cukup baru Jumat, lalu Sabtu, lalu Minggu.
      //  2. Hindari tanggal event (23-25 / tanggal kembar) kalau masih ada pilihan lain.
      //  3. Usahakan cocok dengan tanggal libur departemen pasangan (mis. admin<->packing).
      //  4. Sebar rata supaya tidak semua orang libur di tanggal yang sama.
      candidates.sort((a, b) => {
        const wa = weekdayRestCost(a), wb = weekdayRestCost(b);
        if (wa !== wb) return wa - wb;
        const ba = isEventBlackout(a) ? 1 : 0, bb = isEventBlackout(b) ? 1 : 0;
...
function generateBaseline(dates, bags, hosts, approvedLeaveDates, history) {
  ...
  // bag di hari itu (misal 5 host tapi 2 bag x 2-3 shift = 4-6 slot/hari), kekurangan
  // itu harus disebar rata -- BUKAN selalu jatuh ke bag/shift yang sama tiap hari.
  // Caranya: bangun daftar SEMUA slot (bag x shift) hari itu jadi satu list datar
  // (pakai shift asli tiap bag lewat `bagShifts()`, bukan diseragamkan), lalu rotasi
  // urutan list itu tiap hari (geser sejumlah dateIdx) sebelum diisi greedy.
  // Dengan begitu, kalau memang kurang orang, yang kebagian kosong bukan selalu
  // shift/bag yang sama, tapi bergilir hari demi hari.
  // CATATAN: ini TIDAK menambah kapasitas -- kalau total slot yang dibutuhkan per
  // minggu lebih besar dari total kapasitas host (jumlah host x max_shifts_per_week),
  // akan tetap ada slot kosong. Itu artinya perlu nambah host / kurangi shift lewat
  // menu Bag, bukan cuma soal urutan algoritma.
  dates.forEach((date, dateIdx) => {
    const daySlots = [];
    for (const bag of bags) {
      for (const shift of bagShifts(bag)) daySlots.push({ bag, shift });
    }
    if (!daySlots.length) return;
    const offset = dateIdx % daySlots.length;
    const rotatedSlots = [...daySlots.slice(offset), ...daySlots.slice(0, offset)];

    for (const { bag, shift } of rotatedSlots) {
...
/**
 * Entry point utama. Generate jadwal untuk 1 minggu (weekStart = tanggal Senin/Monday, format YYYY-MM-DD).
 * Semua lokasi (Jakarta & Tangerang) sekarang memakai siklus mingguan yang sama: Senin-Minggu (7 hari).
