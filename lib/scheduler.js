 ...
 *  1b. TAPI host TIDAK BOLEH dapat lebih dari 1 shift di BAG YANG SAMA pada
 *      hari yang sama (walau shift-nya beda, misal Pagi & Siang di bag yang
 *      sama) — secara fisik itu berarti host itu "pegang" akun yang sama
 *      dobel di hari itu, yang bukan maksud dari aturan #1. Ini yang tadinya
 *      jadi bug: nama yang sama muncul 2-3x di bag yang sama pada 1 hari.
...
function isAllowed(hostId, date, shift, bagId, placed, approvedLeaveDates, weeklyShiftCount, maxShiftsByHost) {
  ...
  if (today?.shifts.has(shift)) return false; // bentrok waktu: shift sama, tanggal sama (di bag manapun)
  if (today?.bags.has(bagId)) return false; // sudah pegang bag ini di hari yang sama (shift lain) -> tidak boleh dobel
...
function assignRestDays(dates, deptHosts, approvedLeaveDates, preferredDates = []) {
  ...
  if (!deptHosts.length) return restMap;
...
4. Seorang host TIDAK BOLEH mendapat lebih dari 1 shift di BAG YANG SAMA pada tanggal yang sama (walau shift-nya beda). Host boleh dijadwalkan di 2 bag berbeda pada hari yang sama, tapi tidak boleh dobel di bag yang sama.
